...
- `resources/js/utils/http.ts` centralizes Axios defaults and CSRF retry behavior; it is the private transport of the typed modules under `resources/js/api`, which are the only place HTTP requests are built. File uploads pass `FormData` to those modules like any other payload.

## Forms And Uploads

Choose the form transport from the response contract:

- Use Inertia `useForm` for web routes that redirect back with Laravel validation errors and whose processing state belongs to the whole form.
- Use a typed module under `resources/js/api` for JSON endpoints. Keep pending state in the component, update local state from the response, and report success or failure through the mounted notification UI.
- Use `router.post` directly for action-only Inertia redirects, such as logout, when there is no form validation or processing UI to manage.

Browser API endpoints return JSON validation errors for interactive components such as screenshots, thumbnails, custom content, bug reports, reviews, and notification preferences. Do not wrap those requests in `useForm`: doing so obscures their JSON response contract and does not provide Inertia validation bags.
