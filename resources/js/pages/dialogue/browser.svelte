<script lang="ts">
    ...
    const currentVersionId = $derived(initial.versionId);
    const currentVersionName = $derived(initial.versionName ?? '');
    const currentVersionPublishedAt = $derived(initial.versionPublishedAt ?? null);
    ...
    // Fetch options when gameId/language changes
    $effect(() => {
        optionsLoading = true;
        fetchDialogueOptions({ gameId, versionId: currentVersionId, language })
            .then((data) => {
                options = data;
            })
            .catch(() => {
    ...
    // Fetch version stats for the current version
    $effect(() => {
        fetchDialogueVersionStats(currentVersionId)
            .then((data) => {
                versionStats = data;
            })
            .catch(() => {
    ...
    $effect(() => {
        if (showDuplicates || !debouncedQ.trim()) {
            searchData = null;
            return;
        }
        searchLoading = true;
    ...
    $effect(() => {
        fetchWordFrequency({ versionId: currentVersionId, language })
            .then((data) => {
                wordFrequency = data;
            })
            .catch(() => {
    ...
    // Reset character/context when language changes
    $effect(() => {
        void language;
        selectedCharacterId = '';
        selectedContext = '';
            ...
            <div>
                ...
                {#if currentVersionName}
                    <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                        Current version: {currentVersionName}{currentVersionPublishedAt ? ` (${currentVersionPublishedAt})` : ''}
                    </p>
                {/if}
            </div>
        </div>
