<script lang="ts">
    ...
    import { Alert, Button, Card, Dialog, Stars } from '@/components/ui';
    ...
    const openHistory = async (gameId: number, gameName: string) => {
        try {
            const history = await fetchRaterGameHistory(rater.id, gameId);
            historyModal = { gameName, ratings: history, open: true, error: null };
        } catch (error) {
    ...
    <Dialog open={historyModal.open} onClose={closeHistory} title={historyModal.gameName || 'Rating History'} size="lg">
        ...
        <div class="space-y-6">
            {#if historyModal.error}
                <Alert tone="danger">{historyModal.error}</Alert>
