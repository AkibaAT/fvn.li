<script lang="ts">
  ...
  const toggleLanguagePreference = (isoCode: string) => {
    selectedLanguages = selectedLanguages.includes(isoCode)
      ? selectedLanguages.filter((l) => l !== isoCode)
      : [...selectedLanguages, isoCode];
  };
            ...
            <div class="grid grid-cols-3 gap-3">
              {#each [{ value: 'asap', label: 'As soon as possible', desc: 'Get notified immediately when games are updated', icon: 'icon-bell' }, { value: 'daily', label: 'Daily digest', desc: 'Get a summary of all updates once per day', icon: 'icon-paste' }, { value: 'weekly', label: 'Weekly digest', desc: 'Get a summary of all updates once per week', icon: 'icon-paste' }] as freq (freq.value)}
                <Button
                  ...
                  disabled={savingPrefs}
                  class="rounded-lg border-2 p-3 text-left text-sm transition-colors {notifPrefs.notification_digest ===
                  freq.value
...
{#if activeTab === 'search'}
  <div class="space-y-6">
    ...
    <Card padding="lg">
      ...
      <p class="mb-3 text-sm text-gray-600 dark:text-gray-400">
        Set your preferred languages to auto-filter the games list. When set, the games page will show only games
        available in these languages by default.
      </p>
    ...
    <Card padding="lg">
      ...
      <p class="mb-3 text-sm text-gray-600 dark:text-gray-400">
        Select tags to exclude from game search results by default. Games with any of these tags will be hidden unless
          ...
          {#each ignoredGames as game (game.id)}
            <div class="flex items-center justify-between rounded-lg bg-gray-50 p-2 dark:bg-gray-700/50">
              <Link
                ...
                class="truncate text-sm text-blue-600 hover:underline dark:text-blue-400">{game.name}</Link
              >
              <Button
                type="button"
                variant="link"
