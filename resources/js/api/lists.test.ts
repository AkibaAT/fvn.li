...
describe('lists API module', () => {
    ...
    test('transport rejections propagate to the caller', async () => {
        http.post.mockRejectedValueOnce(new Error('You must be logged in.'));

        await expect(toggleVnListVisibility(5)).rejects.toThrow('You must be logged in.');
    });
});
