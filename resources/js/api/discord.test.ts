...
describe('discord API module', () => {
    ...
    test('propagates transport rejections to the caller', async () => {
        http.get.mockRejectedValueOnce(new Error('Server not found'));

        await expect(fetchDiscordServer(999)).rejects.toThrow('Server not found');
    });
});
