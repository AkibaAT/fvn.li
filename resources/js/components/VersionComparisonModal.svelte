...
{#if isOpen}
    ...
    >
        ...
        <div class="max-h-[90vh] w-full overflow-hidden">
            <!-- Header -->
                                            ...
                                            {#each comparisonData.characters as character (character)}
                                                <tr class="hover:bg-gray-700/50">
                                                    ...
                                                    {#each comparisonData.languages as lang, index (lang.id)}
                                                        ...
                                                        {/if}
                                                        <td class="px-2 py-2 text-right text-sm text-gray-400 tabular-nums"
                                                            >{formatNumber(fromCount)}</td
                                                        >
                                                        <td class="px-2 py-2 text-right text-sm text-gray-100 tabular-nums"
                                                            >{formatNumber(toCount)}</td
                                                        >
                                                        <td class="px-2 py-2 text-right text-sm tabular-nums {getDiffColor(diff)}"
                                                            >{formatDiff(diff)}</td
                                                        >
                                                    {/each}
                                                </tr>
                                        ...
                                        <tfoot class="border-t border-gray-600 font-medium">
                                            <tr>
                                                ...
                                                {#each comparisonData.languages as lang, index (lang.id)}
                                                    ...
                                                    <td class="px-2 py-2 text-right text-sm tabular-nums {getDiffColor(diffTotal)}"
                                                    ...
                                                    <tbody class="divide-y divide-gray-600">
                                                        {#each Object.entries(category.fileTypes) as [extension, typeStats] (extension)}
                                                            <tr>
                                                                ...
                                                                >
                                                                <td class="px-2 py-2 text-right text-sm text-gray-400"
                                                                    >{formatBytesUtil(typeStats.from.size)}</td
                                                                >
                                                                <td class="px-2 py-2 text-right text-sm text-gray-100"
                                                                    >{formatBytesUtil(typeStats.to.size)}</td
                                                                >
