<script lang="ts">
    ...
    const cardClass =
        'group flex flex-col overflow-hidden rounded-xl border border-gray-200/50 bg-white/70 shadow transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg dark:border-gray-700/50 dark:bg-gray-800/70';
    ...
    <Card {id} padding="lg" class={compact ? 'mt-6' : 'mt-6 scroll-mt-28'}>
        ...
        <div class={gridClass}>
            {#each games as game (game.id)}
                <Link href={route('games.show', game.slug)} class={cardClass}>
