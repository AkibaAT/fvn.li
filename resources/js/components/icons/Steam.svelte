<script lang="ts">
    let { class: className = 'h-5 w-5', monochrome = false, ...restProps }: { class?: string; monochrome?: boolean; [key: string]: any } = $props();

    const primaryFill = $derived(monochrome ? 'fill: currentColor' : 'fill: var(--color-steam-primary)');
    const secondaryFill = $derived(monochrome ? 'fill: currentColor' : 'fill: var(--color-steam-secondary)');
</script>
