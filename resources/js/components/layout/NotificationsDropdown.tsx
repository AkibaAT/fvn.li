    ...
    switch (notification.data.type) {
        case 'bug_report_reply':
            return (
                route('dashboard') +
                '?bug_report=' +
                notification.data.bug_report_id
            );
        default:
            return null;
    }
...
export default function NotificationsDropdown() {
    ...
    const { data: items = [], isLoading: loading } = useNotifications({
        enabled: open,
    });
                        ...
                        {loading ? (
                            ...
                            <ul className="divide-y divide-stone-100 dark:divide-gray-700">
                                {items.map((n) => (
                                    <li
                                        key={n.id}
                                        className="rounded-lg p-3 transition-colors hover:bg-stone-50 dark:hover:bg-gray-700/50"
                                    >
