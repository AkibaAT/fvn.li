...
<section class="space-y-6">
    <h2 class="text-2xl font-semibold tracking-tight text-fg">Overview</h2>

    <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
        <Card variant="flat">
            <h3 class="mb-4 text-xl font-semibold text-fg">Games</h3>
            ...
            {#if gameLatestUpdate}
                <div class="mt-4 text-sm">
                    <span class="text-fg-muted">Latest Update:</span><span class="ml-1 text-fg"
                        >{gameLatestUpdate.timeAgo}
                        <span class="text-xs text-fg-faint">({gameLatestUpdate.formattedDate})</span></span
                    >
                </div>
            {/if}
        </Card>
        ...
        <Card variant="flat">
            <h3 class="mb-4 text-xl font-semibold text-fg">Ratings</h3>
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div>
                    <h3 class="mb-3 text-lg font-medium text-fg">All Ratings</h3>
                    <dl class="grid grid-cols-2 gap-4">
                ...
                <div>
                    <h3 class="mb-3 text-lg font-medium text-fg">Listed Games</h3>
                    <dl class="grid grid-cols-2 gap-4">
            ...
            {#if ratingLatest}
                <div class="mt-4 text-sm">
                    <span class="text-fg-muted">Latest Rating:</span><span class="ml-1 text-fg"
                        >{ratingLatest.timeAgo}
                        <span class="text-xs text-fg-faint">({ratingLatest.formattedDate})</span></span
                    >
                </div>
            {/if}
        </Card>
