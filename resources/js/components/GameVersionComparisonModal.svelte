...
<Dialog
    ...
    onClose={closeVersionComparisonDialog}
    title="Version Comparison"
    size="full"
    describedBy="version-comparison-desc"
    class="min-w-80 bg-gray-800 text-gray-100 dark:bg-gray-800"
    bodyClass="p-6"
>
                                ...
                                <tr>
                                    ...
                                    {#each versionComparisonData?.languages || [] as lang, index (lang.id)}
                                        ...
                                        {#if index > 0}
                                            <td class="m-0 w-px bg-gray-600 p-0"><div class="h-full w-px">&nbsp;</div></td>
                                        {/if}
                                        <td class="px-2 py-2 text-right text-gray-400 tabular-nums">{fromTotal ? fromTotal.toLocaleString() : '-'}</td
                                        >
                                        ...
                                        <tbody class="divide-y divide-gray-600">
                                            {#each Object.entries(category.fileTypes || {}) as [extension, typeStats] (extension)}
                                                <tr>
                                                    ...
                                                    >
                                                    <td class="px-2 py-2 text-right text-sm text-gray-100">{formatBytes(typeStats?.to?.size ?? 0)}</td
                                                    >
                                                    <td
