<script lang="ts">
    ...
    interface GameVersion { id: number; version: string; published_at: string; }
    interface GameRating { id: number; game_id: number; user_id: number; rating: number; is_reviewed: boolean; }
    interface Game {
        id: number; name: string; effective_name: string; slug: string; thumb_url?: string;
        optimized_thumbnails?: { default?: { path: string } } | null; is_nsfw: boolean; is_paid: boolean;
        has_demo: boolean; is_on_sale: boolean; min_price?: number; latest_version?: GameVersion;
        game_versions?: GameVersion[]; user_progress?: UserGameProgress[]; ratings?: GameRating[];
        platform?: 'itch_io' | 'steam' | 'other';
    }
    interface UserGameProgress {
        id: number; user_id: number; game_id: number; game_version_id?: number; personal_notes?: string;
        started_at?: string; completed_at?: string; game_version?: GameVersion; receive_updates?: boolean;
    }
    interface VnListEntry {
        id: number; game: Game; sort_order: number; notes?: string; private_notes?: string; started_at?: string;
        completed_at?: string; user_progress?: UserGameProgress; personal_notes?: string; game_version_id?: number;
        game_version?: GameVersion;
    }
    interface AvailableList { id: number; name: string; type: string; }
    interface EntryFormData {
        game_version_id: string; personal_notes: string; private_notes: string; started_at: string; completed_at: string; target_list_id: string;
    }
    interface ListEntryCardProps {
        entry: VnListEntry; handleAttachment?: Attachment<HTMLElement> | null; isOwner: boolean; isDesktopViewport: boolean;
        vnListType: string; availableListsForMove: AvailableList[]; versionHasCharacterStats: Record<number, boolean>;
        editingEntryId: number | null; movingEntryId: number | null; entryFormData: EntryFormData; entryFormLoading: boolean;
        getOptimizedThumbnail: (game: Game) => string; onCompareVersions: (gameId: number, fromVersionId: number, toVersionId: number) => void;
        onRemove: (entryId: number) => void; onStartEditing: (entry: VnListEntry) => void; onCancelEditing: () => void;
        onSaveEntry: (entryId: number) => void; onStartMoving: (entry: VnListEntry) => void; onCancelMoving: () => void;
        onMoveEntry: (entryId: number) => void; onToggleNotification: (game: Game, newStatus: boolean) => void;
    }

    let {
        entry, handleAttachment = null, isOwner, isDesktopViewport, vnListType, availableListsForMove,
        versionHasCharacterStats, editingEntryId, movingEntryId, entryFormData, entryFormLoading, getOptimizedThumbnail,
        onCompareVersions, onRemove, onStartEditing, onCancelEditing, onSaveEntry, onStartMoving, onCancelMoving,
        onMoveEntry, onToggleNotification,
