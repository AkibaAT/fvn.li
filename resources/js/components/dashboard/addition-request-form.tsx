...
import { useSubmitAdditionRequests, SubmissionResult } from '@/hooks/api';
...
export function AdditionRequestForm() {
  ...
  const submitMutation = useSubmitAdditionRequests();
    ...
    <div className="rounded-2xl border border-gray-200/50 bg-white/70 shadow-lg backdrop-blur-xl dark:border-gray-700/50 dark:bg-gray-800/70">
      <div className="p-6">
        ...
        {showSuccessMessage && submissionResults && (
          <div className="mb-4 rounded-lg border border-green-200 bg-green-100 p-4 dark:border-green-800 dark:bg-green-900/20">
            <div className="text-green-700 dark:text-green-300">
              <p className="font-medium">Requests Submitted Successfully!</p>
              <ul className="mt-2 space-y-1 text-sm">
                ...
                {submissionResults.duplicate_count > 0 && (
                  <li>
                    ℹ {submissionResults.duplicate_count} URL(s) already requested by you
                  </li>
                )}
                {submissionResults.already_exists_count &&
                  submissionResults.already_exists_count > 0 && (
                    <li>
                      ℹ {submissionResults.already_exists_count} game(s) already exist on the site
                    </li>
                  )}
        ...
        {submissionResults?.errors && submissionResults.errors.length > 0 && (
          <div className="mb-4 rounded-lg border border-red-200 bg-red-100 p-4 dark:border-red-800 dark:bg-red-900/20">
        ...
        <form onSubmit={submitRequests} className="space-y-4">
          ...
          <div className="flex gap-3">
            <button
              type="submit"
              disabled={submitMutation.isPending || !urls.trim()}
