    ...
    <div class="p-6">
      <div class="mb-4 flex items-center justify-between">
        <h2 class="flex items-center gap-2 text-lg font-semibold text-amber-800 dark:text-amber-300">
          ...
          {#if totalUnread > 0}
            <span
              class="ml-2 inline-flex items-center justify-center rounded-full bg-red-700 px-2 py-0.5 text-xs font-medium text-white"
            >
              {totalUnread} new
            </span>
          {/if}
          ...
          {#each bugReportComments as comment (comment.id)}
            ...
            >
              <div class="mb-1 flex items-center gap-2">
                ...
                {/if}
                <span
                  class="text-xs {comment.is_from_admin
                    ? 'text-gray-600 dark:text-gray-300'
                    : 'text-gray-500 dark:text-gray-400'}"
                >
                  {new Date(comment.created_at).toLocaleDateString()} at {new Date(
                    comment.created_at,
                  ).toLocaleTimeString([], {
                    hour: '2-digit',
                    minute: '2-digit',
