<div>
    {{-- Header Card --}}
    <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-4 md:p-6 border-l-4 border-{{ $vnList->type === 'reading' ? 'blue' : ($vnList->type === 'completed' ? 'green' : ($vnList->type === 'plan_to_read' ? 'yellow' : ($vnList->type === 'on_hold' ? 'orange' : ($vnList->type === 'dropped' ? 'red' : 'gray')))) }}-500 mb-6">
    ...
    @else
        ...
        <div class="hidden lg:flex bg-gray-100 dark:bg-gray-700 rounded-t-lg p-3 font-medium text-sm text-gray-500 dark:text-gray-300 uppercase">
            <div class="w-8"></div> {{-- Drag handle space --}}
            <div class="w-20 mr-2"></div> {{-- Thumbnail space --}}
            <div class="flex-grow">Title</div>
            <div class="w-56">Version</div>
            <div class="w-28">Started</div>
            @if ($vnList->type === 'custom' || $vnList->type === 'completed')
                <div class="w-28">Completed</div>
            @endif
            @if ($isOwner)
                <div class="w-36">Actions</div>
            @endif
        </div>
        ...
        <div id="entries-list" class="space-y-3 text-gray-700 dark:text-gray-300 md:grid md:grid-cols-2 md:gap-3 md:space-y-0 lg:block lg:space-y-3">
            @foreach ($vnList->entries->sortBy('sort_order') as $entry)
                <div data-id="{{ $entry->id }}" class="bg-white dark:bg-gray-800 rounded-lg md:rounded-lg lg:rounded-none shadow-sm">
                    {{-- Desktop View --}}
                    <div class="hidden lg:flex items-center p-3{{ $isOwner ? ' cursor-move' : '' }}">
                        @if ($isOwner)
                            {{-- Drag Handle --}}
                            <div class="w-8 flex">
                        ...
                        <div class="w-20 mr-2">
                            <a href="{{ route('games.show', $entry->game->slug) }}">
                                <img src="{{ $entry->game->getThumbnailUrl('small') }}"
                                     alt="{{ $entry->game->name }}"
                        ...
                        {{-- Title --}}
                        <div class="flex-grow">
                            <a href="{{ route('games.show', $entry->game->slug) }}" class="font-medium text-blue-600 dark:text-blue-400 hover:underline">
                                {{ $entry->game->name }}
                            </a>
                            @php
                                $userProgress = null;
                        ...
                        <div class="w-56">
                            @if ($currentVersion)
                                <div @class([
                                    ...
                                    @include('components.version-info-block', [
                        ...
                        {{-- Actions --}}
                        @if ($isOwner)
                            <div class="w-36 text-sm space-x-2">
                                <button
                    ...
                    <div class="flex lg:hidden p-4 relative{{ $isOwner ? ' cursor-move' : '' }}">
                        @if ($isOwner)
                            <div class="absolute -left-1 top-1/2 transform -translate-y-1/2 flex items-center">
                                <div class="w-6 h-10 flex items-center justify-center bg-gray-100 dark:bg-gray-700 rounded-r-md">
                        ...
                        <div class="flex gap-4 @if ($isOwner) pl-4 @endif">
                            ...
                            {{-- Game Info --}}
                            <div class="flex-1">
                                <a href="{{ route('games.show', $entry->game->slug) }}" class="text-lg font-medium text-blue-600 dark:text-blue-400 hover:underline">
                                    {{ $entry->game->name }}
                                </a>
                                ...
                                @if ($isOwner)
                                    <div class="flex mt-3 text-sm space-x-2">
                                        <button
                                            onclick="toggleEditForm('{{ $entry->id }}')"
                                            class="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-300 cursor-pointer">
                                            Edit
                    ...
                    @if ($isOwner)
                        <div id="edit-form-{{ $entry->id }}" class="hidden border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 p-4">
                            <form action="{{ route('user-progress.update', $entry->game->id) }}" method="POST" class="space-y-4 entry-edit-form">
                                ...
                                <div>
                                    ...
                                    <textarea id="personal_notes-{{ $entry->id }}" name="personal_notes" rows="4" class="mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-md py-2 px-3">{{ $userProgress?->personal_notes }}</textarea>
                                </div>
                        ...
                        {{-- Move Form --}}
                        <div id="move-form-{{ $entry->id }}" class="hidden border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 p-4">
