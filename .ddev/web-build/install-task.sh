...
http_download_curl() {
  ...
  source_url=$2
  header=$3
  retry_opts="--retry 5 --retry-delay 3 --retry-all-errors --connect-timeout 15"
  if [ -z "$header" ]; then
    # shellcheck disable=SC2086
    code=$(curl $retry_opts -w '%{http_code}' -sL -o "$local_file" "$source_url")
  else
    # shellcheck disable=SC2086
    code=$(curl $retry_opts -w '%{http_code}' -sL -H "$header" -o "$local_file" "$source_url")
  fi
  if [ "$code" != "200" ]; then
