...
it('builds a locked down docker command for per-archive optimization', function () {
    ...
    expect($command)->toContain('--network')
        ...
        ->and($command)->toContain('type=bind,source=/host/app/app,target=/srv/app/app,readonly')
        ->and($command)->toContain('type=bind,source=/host/app/vendor,target=/srv/app/vendor,readonly')
        ->and($command)->toContain('/srv/app/bootstrap/cache:rw,nosuid,nodev,noexec,mode=0770,size=16m')
        ->and($command)->not->toContain('type=bind,source=/host/app,target=/srv/app,readonly')
