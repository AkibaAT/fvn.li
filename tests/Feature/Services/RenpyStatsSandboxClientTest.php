...
it('distinguishes a confirmed unsupported archive from transient analyzer failures', function () {
    $sharedPath = storage_path('framework/testing/renpy-analyzer-shared-' . uniqid());
    $archivePath = storage_path('framework/testing/source-archive-' . uniqid() . '.zip');
    File::ensureDirectoryExists(dirname($archivePath));
    File::put($archivePath, 'archive');
    ...
    Http::fakeSequence('stats-runner:8080/*')
        ->push(['message' => 'No stats could be extracted', 'code' => 'unsupported_archive'], 422)
        ->push(['message' => 'Analyzer unavailable'], 503);
    $client = new RenpyStatsSandboxClient;
    try {
        expect($client->extract($archivePath))->toBeNull()
            ->and($client->wasLastExtractionUnsupported())->toBeTrue();
        expect($client->extract($archivePath))->toBeNull()
            ->and($client->wasLastExtractionUnsupported())->toBeFalse();
    } finally {
        File::delete($archivePath);
        File::deleteDirectory($sharedPath);
    }
