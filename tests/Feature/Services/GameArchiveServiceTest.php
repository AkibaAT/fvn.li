...
test('download URL helper parsing and error formatting are deterministic', function () {
    expect(invokeGameArchiveServiceMethod($this->archiveService, 'uploadDownloadEndpoint', ['https://creator.itch.io/game/', 123]))
        ...
        ->toBeNull()
        ->and(invokeGameArchiveServiceMethod($this->archiveService, 'extractDownloadUrlEndpoint', ['<script>window.I = {"generate_download_url":"https:\\/\\/creator.itch.io\\/game\\/download_url"};</script>']))
        ->toBe('https://creator.itch.io/game/download_url')
        ->and(invokeGameArchiveServiceMethod($this->archiveService, 'extractDownloadUrlEndpoint', ['<p>{"generate_download_url":"https:\\/\\/127.0.0.1\\/generate"}</p>']))
        ->toBeNull()
