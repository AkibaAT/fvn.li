    ...
    public static function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                ...
                Section::make('Other Reviews by This User')
                    ->schema([
                        Placeholder::make('user_review_ban_status')
                            ->label('Review Ban Status')
                            ->content(function (ReviewReport $record): string {
                                $author = $record->rating?->user;
                                if (! $author) {
                                    return 'N/A (imported review)';
                                }

                                return $author->is_review_banned ? 'BANNED' : 'Active';
                            }),
                        ...
                        Placeholder::make('user_other_reviews')
                            ...
                            ->content(function (ReviewReport $record): HtmlString {
                                $author = $record->rating?->user;
                                if (! $author) {
                                    return new HtmlString('<em>This is an imported review — no user account linked.</em>');
                                }

                                $reviews = Rating::where('user_id', $author->id)
                                ...
                                foreach ($reviews as $review) {
                                    ...
                                    $rating = $review->rating ? $review->rating . '/5' : 'N/A';
                                    ...
                                    $html .= "<div class=\"font-medium\">{$gameName}{$isCurrent} — {$rating} — {$visible}</div>";
