...
class GameDetail extends Component
{
    ...
    public ?Collection $userLists = null;

    public ?Collection $userListsWithGame = null;

    public ?Collection $userListsWithoutGame = null;

    public ?VnListEntry $currentListEntry = null;

    public ?string $currentListType = null;

    public bool $showAddToListModal = false;

    public bool $showMoveToListModal = false;

    public bool $showRemoveFromListModal = false;

    public bool $showToggleUpdatesModal = false;

    public bool $showToggleAllUpdatesModal = false;

    public bool $showAddToCustomListModal = false;

    public bool $showRemoveFromCustomListModal = false;

    public bool $showEditNotesModal = false;

    public bool $showVersionComparisonModal = false;

    public ?string $personalNotes = null;

    public ?string $selectedCustomListId = null;

    public ?string $selectedListId = null;

    public ?string $selectedListType = null;

    public ?string $selectedListName = null;

    public ?string $selectedListDescription = null;

    public bool $selectedListIsPublic = false;

    public bool $receiveUpdates = false;

    public bool $receiveAllUpdates = false;
    ...
    public function mount(Game $game): void
    {
        ...
        if (Auth::check()) {
            ...
            if ($this->currentListEntry) {
                $this->currentListType = $this->currentListEntry->list->type;
            }

            // Get user's personal notes for this game
            $userProgress = Auth::user()->gameProgress()->where('game_id', $this->game->id)->first();
            if ($userProgress) {
                $this->personalNotes = $userProgress->personal_notes;
                $this->receiveUpdates = $userProgress->receive_updates;
            }
        }
    }
    ...
    public function render(): View
    {
        ...
        if (Auth::check()) {
            ...
            // Check if user wants to receive notifications for this game
            $userProgress = $this->game->userProgress()
                ->where('user_id', Auth::id())
                ->first();

            $this->receiveNotifications = $userProgress ? (bool) $userProgress->receive_updates : false;
        }
        ...
        return view('games.detail.show', [
            ...
            'hasPrice' => $this->game->current_price > 0,
            'currentPrice' => $this->game->current_price,
            'originalPrice' => $this->game->original_price,
