...
class RouteGraphService
{
    ...
    {
        $graph = $this->storedGraph($version, $includeUnreachable);
        if ($graph === null) {
            throw new RuntimeException('Route graph has not been precomputed for the current graph revision.');
        }

        return $graph;
    }

    public function storedGraph(GameVersion $version, bool $includeUnreachable = false): ?array
    {
        $graph = $includeUnreachable
            ? $version->route_graph_unreachable_data
            : $version->route_graph_data;

        return is_array($graph) && ($graph['graph_revision'] ?? null) === self::GRAPH_REVISION
            ? $graph
            : null;
    }
    ...
    public function computeAndStore(GameVersion $version): array
    {
        ...
        $unreachableGraph = $this->computeGraph($version, includeUnreachable: true);
        $version->route_graph_data = $graph;
        $version->route_graph_unreachable_data = $unreachableGraph;
        $version->saveQuietly();
