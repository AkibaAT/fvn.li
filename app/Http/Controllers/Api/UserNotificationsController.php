...
class UserNotificationsController extends Controller
{
    ...
    public function getGameSubscribers(Request $request): JsonResponse
    {
        ...
        if ($notificationType === 'discord') {
            ...
            $discordIds = $users->pluck('socialAccounts.0.provider_id')->filter()->values();
            ...
            // Get users with Steam logins (for browser notifications)
            $query->whereHas('socialAccounts', function ($query) {
                $query->where('provider_name', 'steam');
            });
